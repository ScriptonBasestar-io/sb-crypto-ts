...
export default class Base64 implements Encoder {
  private static readonly _map = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/='

  private _reverseMap: Array<number> | undefined = undefined
  ...
  parse(base64Str: string): WordArray {
    ...
    if (this._reverseMap === undefined) {
      this._reverseMap = []
      for (let j = 0; j < Base64._map.length; j++) {
        this._reverseMap[Base64._map.charCodeAt(j)] = j
      }
    }

    // Ignore padding
    const paddingChar = Base64._map.charAt(64)
    if (paddingChar) {
  ...
  stringify(wordArray: WordArray): string {
    ...
    for (let i = 0; i < wordArray.sigBytes; i += 3) {
      ...
      for (let j = 0; j < 4 && i + j * 0.75 < wordArray.sigBytes; j++) {
        base64Chars.push(Base64._map.charAt((triplet >>> (6 * (3 - j))) & 0x3f))
      }
    }
